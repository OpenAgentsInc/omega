...
pub mod workbench_surface_store;
...
use omega_actions;
pub use omega_actions::{CreateWorktree, NewWorktreeBranchTarget, SwitchWorktree};
...
actions!(
    ...
    [
        ...
        ToggleNewThreadMenu,
        /// Toggles the composer's new-conversation executor dropdown.
        ToggleComposerExecutorMenu,
...
/// Activates one of the first ten visible Omega thread tabs by zero-based index.
#[derive(Clone, PartialEq, Deserialize, JsonSchema, Action)]
#[action(namespace = agent)]
pub struct ActivateOmegaThreadTab(pub usize);
...
fn update_command_palette_filter(cx: &mut App) {
    ...
    CommandPaletteFilter::update_global(cx, |filter, _| {
        ...
        let manage_skills_action = [TypeId::of::<omega_actions::assistant::ManageSkills>()];
        let skill_creator_actions = [
            TypeId::of::<omega_actions::assistant::OpenSkillCreator>(),
            TypeId::of::<omega_actions::assistant::CreateSkillFromUrl>(),
        ];
        ...
        if disable_ai {
            ...
            filter.hide_namespace("omega_predict_onboarding");
            filter.hide_namespace("edit_prediction");

            filter.hide_action_types(&edit_prediction_actions);
            filter.hide_action_types(&[TypeId::of::<omega_actions::OpenOmegaPredictOnboarding>()]);
        } else {
            if agent_enabled {
            ...
            filter.show_namespace("omega_predict_onboarding");
            filter.show_action_types(&[TypeId::of::<omega_actions::OpenOmegaPredictOnboarding>()]);
...
mod tests {
    ...
    fn test_agent_command_palette_visibility(cx: &mut TestAppContext) {
        ...
        cx.update(|cx| {
            ...
            assert!(
                !filter.is_hidden(&omega_actions::assistant::OpenSkillCreator),
                "OpenSkillCreator should be visible by default"
            );
            assert!(
                !filter.is_hidden(&omega_actions::assistant::CreateSkillFromUrl),
                "CreateSkillFromUrl should be visible by default"
            );
            assert!(
                !filter.is_hidden(&omega_actions::assistant::OpenGlobalAgentsMdRules),
                "OpenGlobalAgentsMdRules should be visible by default"
            );
            assert!(
                !filter.is_hidden(&omega_actions::assistant::OpenProjectAgentsMdRules),
