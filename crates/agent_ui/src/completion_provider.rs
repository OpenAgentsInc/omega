...
impl PromptLocalCommand {
    ...
    pub fn description(&self) -> &'static str {
        match self {
            Self::ThumbsUp => {
                "Rate this response as helpful. Sends the current conversation to the Omega team."
            }
            Self::ThumbsDown => {
                "Rate this response as not helpful. Sends the current conversation to the Omega team."
            }
        }
    }
...
fn collect_session_matches(cx: &App) -> Vec<SessionMatch> {
    ...
    let mut entries: Vec<&ThreadMetadata> = store
        .read(cx)
        .entries()
        .filter(|t| !t.archived && t.agent_id == *agent::OMEGA_AGENT_ID)
        .collect();
