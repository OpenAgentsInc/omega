...
use immortal_client::domain::{Event, Tag};
    ...
    loop {
        ...
        let frame: Value = match serde_json::from_str(&text) {
            ...
            Err(error) => {
                send_event(
                    &events,
                    SessionSocketEvent::Diagnostic(format!("session frame is not JSON: {error}")),
                )
                .await?;
                continue;
            }
        ...
        match label.as_str() {
            "AUTH" => {
                let Some(challenge) = frame.get(1).and_then(Value::as_str) else {
                    send_event(
                        ...
                        SessionSocketEvent::Diagnostic(
                            "relay AUTH frame has no challenge".to_owned(),
                        ),
                    )
                    .await?;
                    continue;
                ...
                authenticated = true;
                send_event(&events, SessionSocketEvent::Authenticated).await?;
            }
            "EOSE" => {
                if frame.get(1).and_then(Value::as_str) == Some(SESSION_SUBSCRIPTION_ID) {
                    send_event(&events, SessionSocketEvent::SubscriptionLive).await?;
                }
            }
            "EVENT" => {
                if frame.get(1).and_then(Value::as_str) != Some(SESSION_SUBSCRIPTION_ID) {
                ...
                let raw = match serde_json::to_vec(wrap) {
                    ...
                    Err(error) => {
                        send_event(
                            &events,
                            SessionSocketEvent::Diagnostic(format!(
                                "session wrap could not be serialized: {error}"
                            )),
                        )
                        .await?;
                        continue;
                ...
                match unwrap_mkt_record_raw(&raw, &signer, &swp_profile_support()) {
                    ...
                    Err(error) => {
                        send_event(
                            &events,
                            SessionSocketEvent::Diagnostic(format!(
                                "session wrap was rejected: {error}"
                            )),
                        )
                        .await?;
                    }
            ...
            "NOTICE" => {
                ...
                send_event(
                    &events,
                    SessionSocketEvent::Diagnostic(format!("relay notice: {notice}")),
                )
                .await?;
            }
            _ => {}
