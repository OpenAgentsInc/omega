...
| Surface | Path |
| --- | --- |
| macOS `cargo-bundle` | `crates/omega/Cargo.toml` `package.metadata.bundle-*` icon lists |
| macOS document type | `crates/omega/resources/Document.icns` via `script/bundle-mac` |
...
| About window | `crates/omega/src/zed.rs` embeds channel PNGs |
