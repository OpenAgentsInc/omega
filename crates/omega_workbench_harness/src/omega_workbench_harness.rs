...
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct OutlineBindingFixture {
    pub thread_id: String,
...
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case", tag = "state", content = "message")]
pub enum OutlineLifecycleFixture {
    Loading,
    Ready,
    Streaming,
    Partial(String),
...
#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum OutlineItemStatusFixture {
    Pending,
    Running,
    Completed,
    Failed,
...
#[derive(Clone, Copy, Debug, PartialEq, Eq, PartialOrd, Ord, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum OutlineItemKindFixture {
    File,
    Diff,
    Command,
    Plan,
...
#[derive(Clone, Debug, PartialEq, Eq, PartialOrd, Ord, Serialize, Deserialize)]
#[serde(rename_all = "snake_case", tag = "source", content = "identity")]
pub enum OutlineSourceFixture {
    Message(String),
    ToolCall(String),
    TranscriptEntry(usize),
    WorktreePath { worktree_id: String, path: String },
    Command(String),
    Uri(String),
}
...
pub enum OutlineActionTargetFixture {
    Transcript {
        entry_index: usize,
    },
    File {
...
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct OutlineItemFixture {
    pub id: OutlineItemId,
    pub revision: u64,
    pub sequence: u64,
    pub kind: OutlineItemKindFixture,
    pub status: OutlineItemStatusFixture,
    pub title: String,
    pub source: OutlineSourceFixture,
    pub action_target: OutlineActionTargetFixture,
    pub occurrence_count: u32,
    pub historical: bool,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct OutlineVirtualWindowFixture {
    pub start: usize,
    pub length: usize,
    pub total: usize,
}
...
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct OutlineSnapshotFixture {
    pub binding: OutlineBindingFixture,
    pub revision: u64,
    pub lifecycle: OutlineLifecycleFixture,
    pub view: OutlineViewFixture,
    pub filter: OutlineFilterFixture,
    pub items: Vec<OutlineItemFixture>,
    pub selected_item_id: Option<OutlineItemId>,
    pub anchor_item_id: Option<OutlineItemId>,
    pub virtual_window: OutlineVirtualWindowFixture,
    pub retained_surface_token: String,
    pub ignored_stale_revision_count: u32,
    pub ignored_replay_count: u32,
    pub rejected_conflicting_revision_count: u32,
    pub rejected_foreign_binding_count: u32,
}

#[derive(Clone, Debug, PartialEq, Eq)]
pub struct OutlineObservation {
    pub kind: OutlineItemKindFixture,
    pub status: OutlineItemStatusFixture,
    pub title: String,
    pub source: OutlineSourceFixture,
...
pub struct OutlineUpdate {
    pub binding: OutlineBindingFixture,
    pub revision: u64,
    pub observations: Vec<OutlineObservation>,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum OutlineUpdateEffect {
    Applied,
    StaleIgnored,
    ReplayIgnored,
    ConflictingReplayRejected,
    ForeignBindingRejected,
}

#[derive(Clone, Debug)]
pub struct OutlineProjection {
    binding: OutlineBindingFixture,
    revision: u64,
    lifecycle: OutlineLifecycleFixture,
    view: OutlineViewFixture,
    filter: OutlineFilterFixture,
    items: Vec<OutlineItemFixture>,
    selected_item_id: Option<OutlineItemId>,
    anchor_item_id: Option<OutlineItemId>,
    virtual_start: usize,
    virtual_length: usize,
    retained_surface_token: String,
    next_item_id: u64,
    next_sequence: u64,
    ignored_stale_revision_count: u32,
    ignored_replay_count: u32,
    rejected_conflicting_revision_count: u32,
    rejected_foreign_binding_count: u32,
    last_observations: Vec<OutlineObservation>,
}

impl OutlineProjection {
    pub fn new(binding: OutlineBindingFixture, retained_surface_token: impl Into<String>) -> Self {
        Self {
            binding,
            revision: 0,
            lifecycle: OutlineLifecycleFixture::Loading,
            view: OutlineViewFixture::Events,
            filter: OutlineFilterFixture::All,
            items: Vec::new(),
            selected_item_id: None,
            anchor_item_id: None,
            virtual_start: 0,
            virtual_length: 0,
            retained_surface_token: retained_surface_token.into(),
            next_item_id: 1,
            next_sequence: 1,
            ignored_stale_revision_count: 0,
            ignored_replay_count: 0,
            rejected_conflicting_revision_count: 0,
            rejected_foreign_binding_count: 0,
            last_observations: Vec::new(),
        }
    }

    pub fn apply(&mut self, update: OutlineUpdate) -> OutlineUpdateEffect {
        if update.binding != self.binding {
            self.rejected_foreign_binding_count =
                self.rejected_foreign_binding_count.saturating_add(1);
            return OutlineUpdateEffect::ForeignBindingRejected;
        }
        if update.revision < self.revision {
            self.ignored_stale_revision_count = self.ignored_stale_revision_count.saturating_add(1);
            return OutlineUpdateEffect::StaleIgnored;
        }
        if update.revision == self.revision {
            if update.observations == self.last_observations {
                self.ignored_replay_count = self.ignored_replay_count.saturating_add(1);
                return OutlineUpdateEffect::ReplayIgnored;
            }
            self.rejected_conflicting_revision_count =
                self.rejected_conflicting_revision_count.saturating_add(1);
            return OutlineUpdateEffect::ConflictingReplayRejected;
        }

        for item in &mut self.items {
            item.historical = true;
        }
        let mut observations = Vec::<(OutlineObservation, u32)>::new();
        self.last_observations = update.observations.clone();
        for observation in update.observations {
            if let Some((_, count)) = observations.iter_mut().find(|(existing, _)| {
                existing.kind == observation.kind && existing.source == observation.source
            }) {
                *count = count.saturating_add(1);
            } else {
                observations.push((observation, 1));
            }
        }
        for (observation, count) in observations {
            ...
            if let Some(item) = self
                .items
                .iter_mut()
                .find(|item| item.kind == kind && item.source == source)
            {
                item.revision = update.revision;
                item.status = observation.status;
                item.title = observation.title;
                item.action_target = observation.action_target;
                item.occurrence_count = item.occurrence_count.saturating_add(count);
                ...
                self.items.push(OutlineItemFixture {
                    id: OutlineItemId(format!("outline-{item_id}")),
        ...
        self.lifecycle = OutlineLifecycleFixture::Ready;
        self.reconcile_view_state();
        OutlineUpdateEffect::Applied
    }

    pub fn set_lifecycle(&mut self, lifecycle: OutlineLifecycleFixture) {
        self.lifecycle = lifecycle;
        self.reconcile_view_state();
    }

    pub fn set_view_filter(&mut self, view: OutlineViewFixture, filter: OutlineFilterFixture) {
        self.view = view;
        self.filter = filter;
        self.reconcile_view_state();
    }

    pub fn select(&mut self, item_id: Option<OutlineItemId>) -> bool {
        if item_id.as_ref().is_some_and(|item_id| {
            !self.items.iter().any(|item| {
                &item.id == item_id && outline_item_visible(self.view, self.filter, item)
            })
        }) {
            return false;
        }
        self.selected_item_id = item_id.clone();
        self.anchor_item_id = item_id;
        true
    }

    pub fn set_virtual_window(&mut self, start: usize, length: usize) {
        let total = self.visible_item_count();
        self.virtual_start = start.min(total);
        self.virtual_length = length.min(total.saturating_sub(self.virtual_start));
    }

    pub fn snapshot(&self) -> OutlineSnapshotFixture {
        let total = self.visible_item_count();
        OutlineSnapshotFixture {
            ...
            selected_item_id: self.selected_item_id.clone(),
            anchor_item_id: self.anchor_item_id.clone(),
            virtual_window: OutlineVirtualWindowFixture {
                start: self.virtual_start,
                length: self
                    .virtual_length
                    .min(total.saturating_sub(self.virtual_start)),
                total,
            },
            retained_surface_token: self.retained_surface_token.clone(),
            ignored_stale_revision_count: self.ignored_stale_revision_count,
            ignored_replay_count: self.ignored_replay_count,
            rejected_conflicting_revision_count: self.rejected_conflicting_revision_count,
            rejected_foreign_binding_count: self.rejected_foreign_binding_count,
        }
    }
    ...
    fn visible_item_count(&self) -> usize {
        self.items
            .iter()
            .filter(|item| outline_item_visible(self.view, self.filter, item))
            .count()
    }

    fn reconcile_view_state(&mut self) {
        let is_visible = |item_id: &OutlineItemId| {
            self.items.iter().any(|item| {
                &item.id == item_id && outline_item_visible(self.view, self.filter, item)
            })
        };
        if self
            .selected_item_id
            .as_ref()
            .is_some_and(|item_id| !is_visible(item_id))
        {
        ...
        self.set_virtual_window(self.virtual_start, self.virtual_length);
    }
}
...
pub struct WorkbenchScene {
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub outline_snapshots: Vec<OutlineSnapshotFixture>,
...
impl WorkbenchScene {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        self.validate_outline_snapshots()?;
    ...
    fn validate_outline_snapshots(&self) -> Result<()> {
        unique_ids(
            "outline snapshot thread",
            self.outline_snapshots
                .iter()
                .map(|snapshot| snapshot.binding.thread_id.as_str()),
        )?;

        for snapshot in &self.outline_snapshots {
            let binding = &snapshot.binding;
            let thread = self
                .threads
                .iter()
                ...
                .ok_or_else(|| {
                    anyhow!(
                        "outline snapshot references missing thread {:?}",
                        binding.thread_id
                    )
                })?;
            if thread.repository_id.as_deref() != Some(binding.repository_id.as_str())
            ...
            {
                bail!(
                    "outline snapshot for thread {:?} does not match its repository/worktree binding",
                    binding.thread_id
                );
            }
            let workbench = self
                ...
                .ok_or_else(|| {
                    anyhow!(
                        "outline snapshot for thread {:?} has no workbench projection",
                        binding.thread_id
                    )
                })?;
            if workbench.generation != binding.generation
            ...
            {
                bail!(
                    "outline snapshot for thread {:?} does not match its workbench generation/binding",
                    binding.thread_id
                );
            }
            if snapshot.retained_surface_token.trim().is_empty() {
                bail!("outline retained surface token must not be empty");
            }
            if let OutlineLifecycleFixture::Error(message)
            | OutlineLifecycleFixture::Partial(message) = &snapshot.lifecycle
                && message.trim().is_empty()
            {
                bail!("outline error lifecycle must contain a message");
            }
            ...
            for item in &snapshot.items {
                if item.id.0.trim().is_empty() || !item_ids.insert(item.id.clone()) {
                    bail!("outline item IDs must be non-empty and unique");
                }
                if item.title.trim().is_empty() || item.occurrence_count == 0 {
                    bail!("outline items require a title and non-zero occurrence count");
                }
                if item.revision > snapshot.revision {
                    bail!(
                        "outline item {:?} is newer than snapshot revision {}",
                        item.id,
                        snapshot.revision
                    );
                }
                if previous_sequence.is_some_and(|sequence| sequence >= item.sequence) {
                    bail!("outline items must be strictly ordered by sequence");
                }
                previous_sequence = Some(item.sequence);
                if !source_keys.insert((item.kind, item.source.clone())) {
                    bail!("outline history must deduplicate repeated source identities");
                }
                validate_outline_item(item, binding)?;
            }

            let visible_ids = snapshot
                .items
                .iter()
                .filter(|item| outline_item_visible(snapshot.view, snapshot.filter, item))
                .map(|item| item.id.clone())
                .collect::<Vec<_>>();
            if snapshot.virtual_window.total != visible_ids.len()
                || snapshot.virtual_window.start > snapshot.virtual_window.total
                || snapshot.virtual_window.length
                    > snapshot
                        .virtual_window
                        .total
                        .saturating_sub(snapshot.virtual_window.start)
            {
                bail!("outline virtual window does not match the filtered projection");
            }
            if let Some(selected_item_id) = &snapshot.selected_item_id
                && !visible_ids.contains(selected_item_id)
            {
                bail!("outline selection references an item outside the filtered view");
            }
            if let Some(anchor_item_id) = &snapshot.anchor_item_id
                && !visible_ids.contains(anchor_item_id)
            {
                bail!("outline anchor references an item outside the filtered view");
            }
        }
        Ok(())
    }

    pub fn active_outline_snapshot(&self) -> Option<&OutlineSnapshotFixture> {
        let active_thread_id = self.active_thread_id.as_deref()?;
        self.outline_snapshots
            .iter()
            .find(|snapshot| snapshot.binding.thread_id == active_thread_id)
...
fn outline_item_visible(
    view: OutlineViewFixture,
    filter: OutlineFilterFixture,
    item: &OutlineItemFixture,
) -> bool {
    let view_matches = match view {
...
fn validate_outline_item(item: &OutlineItemFixture, binding: &OutlineBindingFixture) -> Result<()> {
    match &item.source {
        OutlineSourceFixture::Message(id)
        | OutlineSourceFixture::ToolCall(id)
        | OutlineSourceFixture::Command(id)
        | OutlineSourceFixture::Uri(id)
            if id.trim().is_empty() =>
        {
            bail!("outline source identity must not be empty");
        }
        OutlineSourceFixture::WorktreePath { worktree_id, path } => {
            if worktree_id != &binding.worktree_id || path.trim().is_empty() {
                bail!("outline worktree source must match its binding and contain a path");
            }
        }
        _ => {}
    }
    match &item.action_target {
        OutlineActionTargetFixture::File {
            worktree_id, path, ..
        }
        | OutlineActionTargetFixture::Diff { worktree_id, path } => {
            if worktree_id != &binding.worktree_id || path.trim().is_empty() {
                bail!("outline file action must target its bound worktree");
            }
        }
        OutlineActionTargetFixture::Terminal { terminal_id } if terminal_id.trim().is_empty() => {
            bail!("outline terminal action target must not be empty");
        }
        OutlineActionTargetFixture::Url { uri } if uri.trim().is_empty() => {
            bail!("outline URL action target must not be empty");
        }
        _ => {}
    }
    let compatible = match (&item.kind, &item.source, &item.action_target) {
        (
            OutlineItemKindFixture::File,
            OutlineSourceFixture::WorktreePath { .. },
            OutlineActionTargetFixture::File { .. },
        )
        | (
            OutlineItemKindFixture::Diff,
            OutlineSourceFixture::TranscriptEntry(_),
            OutlineActionTargetFixture::Diff { .. },
        )
        | (
            OutlineItemKindFixture::Command,
            OutlineSourceFixture::Command(_),
            OutlineActionTargetFixture::Terminal { .. },
        )
        | (
            OutlineItemKindFixture::Url,
            OutlineSourceFixture::Uri(_),
            OutlineActionTargetFixture::Url { .. },
        )
        | (
            OutlineItemKindFixture::Message,
            OutlineSourceFixture::Message(_),
            OutlineActionTargetFixture::Transcript { .. },
        )
        | (
            OutlineItemKindFixture::ToolCall,
            OutlineSourceFixture::ToolCall(_),
            OutlineActionTargetFixture::Transcript { .. },
        ) => true,
        (
            OutlineItemKindFixture::Plan
    ...
    if !compatible {
        bail!(
            "outline item {:?} has an incompatible kind, source, and action target",
            item.id
        );
    }
    Ok(())
...
pub const WORKBENCH_OUTLINE_PIXEL_SCENES: [&str; 14] = [
    "omega_workbench_outline_empty",
    "omega_workbench_outline_combined",
    "omega_workbench_outline_artifacts",
    "omega_workbench_outline_events",
    "omega_workbench_outline_active_filter",
    "omega_workbench_outline_failed_filter",
    "omega_workbench_outline_selected_action",
    "omega_workbench_outline_deduplicated_history",
    "omega_workbench_outline_virtualized",
    "omega_workbench_outline_stale_replay",
...
pub const WORKBENCH_SHELL_PIXEL_SCENES: [&str; 92] = [
...
pub const WORKBENCH_OUTLINE_REGIONS: &[CaptureRegionSpec] = &[CaptureRegionSpec::selector_union(
    "artifact-event-outline",
    &["omega.workbench.outline"],
    8,
)];
...
pub const HERMETIC_SCENES: &[SceneSpec] = &[
    ...
    SceneSpec {
        name: "omega_workbench_outline_empty",
        phase: ScenePhase::Recording,
        viewport: ViewportFixture::new(1200, 720, 2000),
        fixture_version: 2,
        pixel_policy: APPLE_SILICON_METAL_POLICY,
        regions: WORKBENCH_OUTLINE_REGIONS,
    },
    SceneSpec {
        name: "omega_workbench_outline_combined",
        phase: ScenePhase::Recording,
        viewport: ViewportFixture::new(1200, 720, 2000),
        fixture_version: 2,
        pixel_policy: APPLE_SILICON_METAL_POLICY,
    ...
    SceneSpec {
        name: "omega_workbench_outline_artifacts",
        phase: ScenePhase::Recording,
        viewport: ViewportFixture::new(1200, 720, 2000),
        fixture_version: 2,
        pixel_policy: APPLE_SILICON_METAL_POLICY,
    ...
    SceneSpec {
        name: "omega_workbench_outline_events",
        phase: ScenePhase::Recording,
        viewport: ViewportFixture::new(1200, 720, 2000),
        fixture_version: 2,
        pixel_policy: APPLE_SILICON_METAL_POLICY,
    ...
    SceneSpec {
        name: "omega_workbench_outline_active_filter",
        phase: ScenePhase::Recording,
        viewport: ViewportFixture::new(1200, 720, 2000),
        fixture_version: 2,
        pixel_policy: APPLE_SILICON_METAL_POLICY,
    ...
    SceneSpec {
        name: "omega_workbench_outline_failed_filter",
        phase: ScenePhase::Recording,
        viewport: ViewportFixture::new(1200, 720, 2000),
        fixture_version: 2,
        pixel_policy: APPLE_SILICON_METAL_POLICY,
    ...
    SceneSpec {
        name: "omega_workbench_outline_selected_action",
        phase: ScenePhase::Recording,
        viewport: ViewportFixture::new(1200, 720, 2000),
        fixture_version: 2,
        pixel_policy: APPLE_SILICON_METAL_POLICY,
    ...
    SceneSpec {
        name: "omega_workbench_outline_deduplicated_history",
        phase: ScenePhase::Recording,
        viewport: ViewportFixture::new(1200, 720, 2000),
        fixture_version: 2,
        pixel_policy: APPLE_SILICON_METAL_POLICY,
    ...
    SceneSpec {
        name: "omega_workbench_outline_virtualized",
        phase: ScenePhase::Recording,
        viewport: ViewportFixture::new(1200, 720, 2000),
        fixture_version: 2,
        pixel_policy: APPLE_SILICON_METAL_POLICY,
    ...
    SceneSpec {
        name: "omega_workbench_outline_stale_replay",
        phase: ScenePhase::Recording,
        viewport: ViewportFixture::new(1200, 720, 2000),
        fixture_version: 2,
        pixel_policy: APPLE_SILICON_METAL_POLICY,
    ...
    SceneSpec {
        name: "omega_workbench_outline_offline",
        phase: ScenePhase::Recording,
        viewport: ViewportFixture::new(1200, 720, 2000),
        fixture_version: 2,
        pixel_policy: APPLE_SILICON_METAL_POLICY,
    ...
    SceneSpec {
        name: "omega_workbench_outline_reconnecting",
        phase: ScenePhase::Recording,
        viewport: ViewportFixture::new(1200, 720, 2000),
        fixture_version: 2,
        pixel_policy: APPLE_SILICON_METAL_POLICY,
    ...
    SceneSpec {
        name: "omega_workbench_outline_error",
        phase: ScenePhase::Recording,
        viewport: ViewportFixture::new(1200, 720, 2000),
        fixture_version: 2,
        pixel_policy: APPLE_SILICON_METAL_POLICY,
    ...
    SceneSpec {
        name: "omega_workbench_outline_narrow_foreign_binding",
        phase: ScenePhase::Recording,
        viewport: ViewportFixture::new(910, 720, 2000),
        fixture_version: 2,
        pixel_policy: APPLE_SILICON_METAL_POLICY,
...
pub fn workbench_outline_scene(name: &str) -> Result<WorkbenchScene> {
    let spec = scene_spec(name).ok_or_else(|| anyhow!("unknown workbench scene {name:?}"))?;
    if !name.starts_with("omega_workbench_outline_") {
        bail!("{name:?} is not an artifact/event outline scene");
    }

    let active_binding = OutlineBindingFixture {
        thread_id: "active-thread".into(),
        repository_id: "visual-repository".into(),
        worktree_id: "visual-worktree".into(),
        generation: 11,
    };
    let foreign_binding = OutlineBindingFixture {
        thread_id: "foreign-thread".into(),
        repository_id: "foreign-repository".into(),
        worktree_id: "foreign-worktree".into(),
        generation: 4,
    };
    let active_snapshot = outline_fixture_for_scene(name, active_binding.clone())?;
    let mut foreign_projection =
        OutlineProjection::new(foreign_binding.clone(), "foreign-outline-surface");
    foreign_projection.apply(OutlineUpdate {
        binding: foreign_binding.clone(),
        revision: 7,
        observations: vec![outline_observation(
            OutlineItemKindFixture::ToolCall,
            OutlineItemStatusFixture::Running,
            "Foreign tool call",
            OutlineSourceFixture::ToolCall("foreign-tool-call".into()),
            OutlineActionTargetFixture::Transcript { entry_index: 1 },
        )],
    });
    foreign_projection.set_virtual_window(0, 20);
    ...
    scene.repositories = vec![
        RepositoryFixture {
            ...
            worktrees: vec![WorktreeFixture {
                ...
                branch: None,
                git_state: Some(WorktreeGitStateFixture::NoGit),
                dirty_files: 0,
                conflicts: 0,
                ahead: 0,
                behind: 0,
        ...
        RepositoryFixture {
            id: "foreign-repository".into(),
            project_id: "visual-project".into(),
            worktrees: vec![WorktreeFixture {
                id: "foreign-worktree".into(),
                branch: None,
                git_state: Some(WorktreeGitStateFixture::NoGit),
                dirty_files: 0,
                conflicts: 0,
    ...
    scene.active_thread_id = Some(active_binding.thread_id.clone());
    for surface in &mut scene.surfaces {
        surface.available = matches!(
            surface.id,
            WorkSurfaceId::Files
                | WorkSurfaceId::Search
                | WorkSurfaceId::Terminal
                | WorkSurfaceId::Plan
        );
    }
    ...
    scene.outline_snapshots = vec![active_snapshot, foreign_projection.snapshot()];
    scene.validate()?;
    Ok(scene)
}

fn outline_fixture_for_scene(
    name: &str,
    binding: OutlineBindingFixture,
) -> Result<OutlineSnapshotFixture> {
    let mut projection = OutlineProjection::new(binding.clone(), "active-outline-surface");
    if name != "omega_workbench_outline_empty" {
        projection.apply(OutlineUpdate {
            binding: binding.clone(),
            revision: if name == "omega_workbench_outline_selected_action" {
                5
            } else {
                2
            },
            observations: if name == "omega_workbench_outline_selected_action" {
                outline_frontdoor_observations(&binding.worktree_id)?
            } else {
                outline_base_observations(&binding.worktree_id)
            },
        });
    }

    match name {
        "omega_workbench_outline_empty" | "omega_workbench_outline_combined" => {}
        "omega_workbench_outline_artifacts" => {
            projection.set_view_filter(OutlineViewFixture::Artifacts, OutlineFilterFixture::All);
        }
        "omega_workbench_outline_events" => {
            projection.set_view_filter(OutlineViewFixture::Events, OutlineFilterFixture::All);
        }
        "omega_workbench_outline_active_filter" => {
            projection.set_view_filter(OutlineViewFixture::Events, OutlineFilterFixture::Active);
        }
        "omega_workbench_outline_failed_filter" => {
            projection.set_view_filter(OutlineViewFixture::Events, OutlineFilterFixture::Problems);
        }
        "omega_workbench_outline_selected_action" => {
            projection.set_view_filter(OutlineViewFixture::Artifacts, OutlineFilterFixture::All);
            let selected = projection
                ...
                .items
                .into_iter()
                .find(|item| item.kind == OutlineItemKindFixture::Diff)
                .context("outline fixture has no Diff item")?
                .id;
            if !projection.select(Some(selected)) {
                bail!("outline fixture could not select its Diff action");
            }
            for _ in 0..3 {
                if projection.apply(OutlineUpdate {
                    binding: binding.clone(),
                    revision: 5,
                    observations: outline_frontdoor_observations(&binding.worktree_id)?,
                }) != OutlineUpdateEffect::ReplayIgnored
                {
                    bail!("front-door burst replay was not ignored");
                }
            }
        }
        "omega_workbench_outline_deduplicated_history" => {
            projection.set_view_filter(OutlineViewFixture::Artifacts, OutlineFilterFixture::All);
            let repeated = outline_observation(
                OutlineItemKindFixture::File,
                OutlineItemStatusFixture::Completed,
                "Updated src/workbench.rs",
                OutlineSourceFixture::WorktreePath {
                    worktree_id: binding.worktree_id.clone(),
                    path: "src/workbench.rs".into(),
                },
                OutlineActionTargetFixture::File {
                    worktree_id: binding.worktree_id.clone(),
                    path: "src/workbench.rs".into(),
            ...
            projection.apply(OutlineUpdate {
                binding,
                revision: 3,
                observations: vec![repeated.clone(), repeated],
            });
        }
        "omega_workbench_outline_virtualized" => {
            let observations = (0..40)
                .map(|index| {
                    outline_observation(
                        ...
                        OutlineSourceFixture::ToolCall(format!("command-{index}")),
                        OutlineActionTargetFixture::Transcript {
                            entry_index: index + 4,
                        },
                    )
            ...
            projection.set_virtual_window(12, 8);
        }
        "omega_workbench_outline_stale_replay" => {
            let replay_observations = outline_base_observations(&binding.worktree_id);
            projection.apply(OutlineUpdate {
                binding: binding.clone(),
                revision: 1,
        ...
        "omega_workbench_outline_offline" => {
            projection.set_lifecycle(OutlineLifecycleFixture::Stale);
        }
        "omega_workbench_outline_reconnecting" => {
            projection.set_lifecycle(OutlineLifecycleFixture::Reconnecting);
        }
        "omega_workbench_outline_error" => {
            projection.set_lifecycle(OutlineLifecycleFixture::Error(
                "Could not refresh thread outline".into(),
            ));
        }
        "omega_workbench_outline_narrow_foreign_binding" => {
            let mut foreign = binding;
            foreign.thread_id = "foreign-thread".into();
            foreign.repository_id = "foreign-repository".into();
            foreign.worktree_id = "foreign-worktree".into();
            foreign.generation = 4;
            projection.apply(OutlineUpdate {
                binding: foreign,
                revision: 9,
                observations: Vec::new(),
        ...
        _ => bail!("unknown artifact/event outline scene {name:?}"),
    }
    if name != "omega_workbench_outline_virtualized" {
        projection.set_virtual_window(0, usize::MAX);
    }
    Ok(projection.snapshot())
}

fn outline_base_observations(worktree_id: &str) -> Vec<OutlineObservation> {
    vec![
        outline_observation(
            OutlineItemKindFixture::ToolCall,
            OutlineItemStatusFixture::Completed,
            "Read workbench file",
            OutlineSourceFixture::ToolCall("tool-file".into()),
            OutlineActionTargetFixture::Transcript { entry_index: 0 },
        ),
        outline_observation(
            OutlineItemKindFixture::File,
            OutlineItemStatusFixture::Completed,
        ...
        outline_observation(
            OutlineItemKindFixture::ToolCall,
            OutlineItemStatusFixture::Running,
            "Update workbench diff",
            OutlineSourceFixture::ToolCall("tool-diff".into()),
            OutlineActionTargetFixture::Transcript { entry_index: 1 },
        ),
        outline_observation(
            OutlineItemKindFixture::Diff,
            OutlineItemStatusFixture::Running,
            "Review workbench diff",
            OutlineSourceFixture::TranscriptEntry(1),
            OutlineActionTargetFixture::Diff {
                worktree_id: worktree_id.into(),
                path: "src/workbench.rs".into(),
            },
        ...
        outline_observation(
            ...
            OutlineItemStatusFixture::Completed,
            "Agent response",
            OutlineSourceFixture::Message("message-2".into()),
            OutlineActionTargetFixture::Transcript { entry_index: 2 },
        ),
        outline_observation(
            OutlineItemKindFixture::ToolCall,
            OutlineItemStatusFixture::Failed,
            "Failed test command",
            OutlineSourceFixture::ToolCall("tool-failed".into()),
            OutlineActionTargetFixture::Transcript { entry_index: 3 },
        ),
    ]
}

fn outline_frontdoor_observations(worktree_id: &str) -> Result<Vec<OutlineObservation>> {
    let mut base = outline_base_observations(worktree_id).into_iter();
    let file_event = base
        .next()
        .context("front-door fixture has no file event")?;
    let file_artifact = base
        .next()
        .context("front-door fixture has no file artifact")?;
    let diff_event = base
        .next()
        .context("front-door fixture has no diff event")?;
    let diff_artifact = base
        .next()
        .context("front-door fixture has no diff artifact")?;
    let message = base.next().context("front-door fixture has no message")?;
    let failed_event = base
        .next()
        .context("front-door fixture has no failed event")?;
    anyhow::ensure!(
        base.next().is_none(),
        "front-door fixture has unexpected rows"
    );
    Ok(vec![
        file_event,
        outline_observation(
            ...
            OutlineItemStatusFixture::Completed,
            "Tool call completed",
            OutlineSourceFixture::TranscriptEntry(0),
            OutlineActionTargetFixture::Transcript { entry_index: 0 },
        ),
        file_artifact,
        diff_event,
        ...
        outline_observation(
            ...
            OutlineItemStatusFixture::Failed,
            "Tool result",
            OutlineSourceFixture::TranscriptEntry(3),
            OutlineActionTargetFixture::Transcript { entry_index: 3 },
        ),
        outline_observation(
            OutlineItemKindFixture::Error,
...
fn outline_observation(
    kind: OutlineItemKindFixture,
    status: OutlineItemStatusFixture,
    title: &str,
    source: OutlineSourceFixture,
    action_target: OutlineActionTargetFixture,
) -> OutlineObservation {
    OutlineObservation {
        kind,
        status,
...
pub fn prove_outline_surface(
    scene: &WorkbenchScene,
    actual: &OutlineSnapshotFixture,
) -> Result<Vec<ProofCheck>> {
    scene.validate()?;
    let expected = scene
        .active_outline_snapshot()
        .context("outline proof scene has no active snapshot")?;
    let mut checks = Vec::new();

    require_outline_match(
        "outline-thread-worktree-binding",
        &expected.binding,
        &actual.binding,
        &mut checks,
    )?;
    ...
    require_outline_match(
        "outline-view-filter",
        &(expected.view, expected.filter),
        &(actual.view, actual.filter),
        &mut checks,
    )?;
    require_outline_match(
        "outline-stable-item-identities",
        &expected.items,
        &actual.items,
        &mut checks,
    )?;
    require_outline_match(
        "outline-selection-anchor",
        &(&expected.selected_item_id, &expected.anchor_item_id),
        &(&actual.selected_item_id, &actual.anchor_item_id),
        &mut checks,
    )?;
    require_outline_match(
        "outline-virtual-window",
        &expected.virtual_window,
        &actual.virtual_window,
        &mut checks,
    )?;
    require_outline_match(
        "outline-action-targets",
        &expected
            .items
            .iter()
            .map(|item| (&item.id, &item.action_target))
            .collect::<Vec<_>>(),
        &actual
            .items
            .iter()
    ...
    require_outline_match(
        "outline-deduplicated-history",
        &expected
            .items
            .iter()
            .map(|item| (&item.id, item.occurrence_count, item.historical))
            .collect::<Vec<_>>(),
        &actual
            .items
            .iter()
    ...
    require_outline_match(
        "outline-rejected-update-counts",
        &(
        ...
        &(
            actual.ignored_stale_revision_count,
            actual.ignored_replay_count,
            actual.rejected_conflicting_revision_count,
            actual.rejected_foreign_binding_count,
        ),
        &mut checks,
    )?;
    require_outline_match(
        "outline-retained-surface-entity",
        &expected.retained_surface_token,
        &actual.retained_surface_token,
        &mut checks,
    )?;

    let foreign_sources = scene
        .outline_snapshots
        .iter()
        .filter(|snapshot| snapshot.binding.thread_id != expected.binding.thread_id)
        .flat_map(|snapshot| snapshot.items.iter().map(|item| item.source.clone()))
        .collect::<BTreeSet<_>>();
    let leaked_sources = actual
        .items
        .iter()
        .map(|item| item.source.clone())
        .filter(|source| foreign_sources.contains(source))
        .collect::<Vec<_>>();
    if !leaked_sources.is_empty() {
        let detail = format!("active outline leaked foreign sources {leaked_sources:?}");
        checks.push(ProofCheck::failed("outline-no-foreign-items", &detail));
        bail!("{detail}");
    }
    checks.push(ProofCheck::passed("outline-no-foreign-items"));

    Ok(checks)
}

fn require_outline_match<T>(
    name: &str,
    expected: &T,
    actual: &T,
    checks: &mut Vec<ProofCheck>,
        ...
        .map(|(thread_id, thread)| {
            (
                ...
                conformance::ThreadState {
                    ...
                    outline: conformance_outline(&thread.outline),
                },
            )
        })
        .collect();
...
fn conformance_transition(
    ...
    match transition {
        ...
        projection::ProjectionTransition::ReceiveOutlineProjection {
            thread_id,
            generation,
            binding,
            revision,
            ...
            artifact_ids,
            lifecycle,
        } => conformance::Transition::ReceiveOutlineProjection {
            thread_id: conformance::ThreadId(thread_id.clone()),
            generation: *generation,
            binding: binding.as_ref().map(conformance_binding),
            revision: *revision,
            ...
            artifact_ids: artifact_ids
                ...
                .map(conformance::OutlineItemId)
                .collect(),
            lifecycle: conformance_outline_lifecycle(*lifecycle),
        },
        projection::ProjectionTransition::SetOutlineView { thread_id, view } => {
            conformance::Transition::SetOutlineView {
                thread_id: conformance::ThreadId(thread_id.clone()),
                view: conformance_outline_view(*view),
            }
        }
        projection::ProjectionTransition::SetOutlineFilter { thread_id, filter } => {
            conformance::Transition::SetOutlineFilter {
                thread_id: conformance::ThreadId(thread_id.clone()),
                filter: conformance_outline_filter(*filter),
            }
        }
        projection::ProjectionTransition::SelectOutlineItem { thread_id, item_id } => {
            conformance::Transition::SelectOutlineItem {
                thread_id: conformance::ThreadId(thread_id.clone()),
                item_id: item_id.clone().map(conformance::OutlineItemId),
            }
        }
        projection::ProjectionTransition::SetOutlineAnchor { thread_id, item_id } => {
            conformance::Transition::SetOutlineAnchor {
                thread_id: conformance::ThreadId(thread_id.clone()),
                item_id: item_id.clone().map(conformance::OutlineItemId),
            }
        }
        projection::ProjectionTransition::SetOutlineVirtualWindow {
            thread_id,
            start,
            len,
        } => conformance::Transition::SetOutlineVirtualWindow {
            thread_id: conformance::ThreadId(thread_id.clone()),
            start: *start,
            len: *len,
        },
        projection::ProjectionTransition::SetOutlineCollapsed {
            thread_id,
            collapsed,
        } => conformance::Transition::SetOutlineCollapsed {
            thread_id: conformance::ThreadId(thread_id.clone()),
            collapsed: *collapsed,
        },
...
fn conformance_outline(
    outline: &projection::ThreadOutlineProjection,
) -> conformance::ThreadOutlineState {
    conformance::ThreadOutlineState {
        ...
        event_ids: outline
            ...
            .map(conformance::OutlineItemId)
            .collect(),
        artifact_ids: outline
            .artifact_ids
            .iter()
            .cloned()
            .map(conformance::OutlineItemId)
            .collect(),
        lifecycle: conformance_outline_lifecycle(outline.lifecycle),
        view: conformance_outline_view(outline.view),
        filter: conformance_outline_filter(outline.filter),
        selected_id: outline.selected_id.clone().map(conformance::OutlineItemId),
        anchor_id: outline.anchor_id.clone().map(conformance::OutlineItemId),
        virtual_start: outline.virtual_start,
        virtual_len: outline.virtual_len,
        collapsed: outline.collapsed,
    }
}

fn conformance_outline_view(view: projection::OutlineView) -> conformance::OutlineView {
    match view {
        projection::OutlineView::Events => conformance::OutlineView::Events,
        projection::OutlineView::Artifacts => conformance::OutlineView::Artifacts,
    }
}

fn conformance_outline_filter(filter: projection::OutlineFilter) -> conformance::OutlineFilter {
    match filter {
        projection::OutlineFilter::All => conformance::OutlineFilter::All,
        projection::OutlineFilter::Active => conformance::OutlineFilter::Active,
        projection::OutlineFilter::Errors => conformance::OutlineFilter::Errors,
    }
}

fn conformance_outline_lifecycle(
    lifecycle: projection::OutlineLifecycle,
) -> conformance::OutlineLifecycle {
    match lifecycle {
        projection::OutlineLifecycle::Empty => conformance::OutlineLifecycle::Empty,
        projection::OutlineLifecycle::Loading => conformance::OutlineLifecycle::Loading,
        projection::OutlineLifecycle::Streaming => conformance::OutlineLifecycle::Streaming,
        projection::OutlineLifecycle::Partial => conformance::OutlineLifecycle::Partial,
        projection::OutlineLifecycle::Ready => conformance::OutlineLifecycle::Ready,
        projection::OutlineLifecycle::Error => conformance::OutlineLifecycle::Error,
        projection::OutlineLifecycle::Stale => conformance::OutlineLifecycle::Stale,
        projection::OutlineLifecycle::Reconnecting => conformance::OutlineLifecycle::Reconnecting,
    }
}
...
fn conformance_reject_code(
    ...
    match error {
        ...
        ProjectionError::UnknownOutlineItem(_) => RejectCode::UnknownOutlineItem,
        ProjectionError::InvalidOutlineWindow { .. } => RejectCode::InvalidOutlineWindow,
        ProjectionError::ConflictingOutlineRevision(_) => RejectCode::ConflictingOutlineRevision,
...
mod tests {
        ...
        for scenario in WorkbenchConsistencyScenario::ALL {
            ...
            match scenario {
                ...
                WorkbenchConsistencyScenario::StaleCompletion => {
                    ...
                    assert_eq!(thread.outline, conformance::ThreadOutlineState::default());
    ...
    #[test]
    fn conflicting_outline_revision_is_rejected_without_mutating_projection() {
        let mut recorder = WorkbenchTransitionRecorder::new();
        let surfaces = projection::WorkSurface::FALLBACK_ORDER.to_vec();
        recorder.apply(open_bound_thread("thread-a", "worktree-a", &surfaces));
        recorder.apply(projection::ProjectionTransition::ReceiveOutlineProjection {
            thread_id: "thread-a".into(),
            generation: 0,
            binding: Some(production_binding("worktree-a")),
            revision: 1,
            ...
            lifecycle: projection::OutlineLifecycle::Ready,
        });
        let accepted = recorder.state.clone();
        recorder.apply(projection::ProjectionTransition::ReceiveOutlineProjection {
            thread_id: "thread-a".into(),
            generation: 0,
            binding: Some(production_binding("worktree-a")),
            revision: 1,
            event_ids: vec!["event-conflict".into()],
            artifact_ids: vec!["artifact-1".into()],
            lifecycle: projection::OutlineLifecycle::Ready,
        });
        assert_eq!(recorder.state, accepted);

        let trace = recorder
            .finish()
            .expect("conflicting outline trace conforms");
        assert_eq!(
            trace.steps.last().map(|step| step.observed_effect),
            Some(conformance::TransitionEffect::Rejected {
                code: conformance::RejectCode::ConflictingOutlineRevision,
            })
        );
    }
    ...
    fn production_trace_adapter_covers_every_critical_action() {
        ...
        recorder.apply(projection::ProjectionTransition::SetOutlineView {
            thread_id: "thread-a".into(),
            view: projection::OutlineView::Artifacts,
        });
        recorder.apply(projection::ProjectionTransition::SetOutlineFilter {
            thread_id: "thread-a".into(),
            filter: projection::OutlineFilter::All,
        });
        recorder.apply(projection::ProjectionTransition::SelectOutlineItem {
            thread_id: "thread-a".into(),
            item_id: Some("artifact-1".into()),
        });
        recorder.apply(projection::ProjectionTransition::SetOutlineAnchor {
            thread_id: "thread-a".into(),
            item_id: Some("artifact-1".into()),
        });
        recorder.apply(projection::ProjectionTransition::SetOutlineVirtualWindow {
            thread_id: "thread-a".into(),
            start: 0,
            len: 1,
        });
        recorder.apply(projection::ProjectionTransition::SetOutlineCollapsed {
            thread_id: "thread-a".into(),
            collapsed: true,
        });
        recorder.apply(projection::ProjectionTransition::RequestSurface {
    ...
    fn plan_scene_catalog_models_typed_lifecycle_history_and_thread_isolation() -> Result<()> {
        ...
        let selected_action = workbench_outline_scene("omega_workbench_outline_selected_action")?;
        let event_kinds = selected_action
            .active_outline_snapshot()
            .context("selected-action outline fixture")?
            .items
            .iter()
            .filter(|item| {
                outline_item_visible(OutlineViewFixture::Events, OutlineFilterFixture::All, item)
            })
            .map(|item| item.kind)
            .collect::<Vec<_>>();
        assert_eq!(
    ...
    #[test]
    fn outline_scene_catalog_covers_typed_projection_and_isolation() -> Result<()> {
        for name in WORKBENCH_OUTLINE_PIXEL_SCENES {
            let spec = scene_spec(name).context("registered outline scene")?;
            assert_eq!(spec.fixture_version, 2);
            assert_eq!(spec.regions, WORKBENCH_OUTLINE_REGIONS);
            assert!(WORKBENCH_SHELL_PIXEL_SCENES.contains(&name));
            let scene = workbench_outline_scene(name)?;
            assert_eq!(scene.outline_snapshots.len(), 2);
            let active = scene
                .active_outline_snapshot()
                .context("outline fixture has no active snapshot")?;
            let foreign = scene
                .outline_snapshots
                .iter()
                .find(|snapshot| snapshot.binding.thread_id != active.binding.thread_id)
                .context("outline fixture has no foreign snapshot")?;
            assert_ne!(active.binding.thread_id, foreign.binding.thread_id);
            assert_ne!(active.binding.worktree_id, foreign.binding.worktree_id);
            assert_ne!(
                active.retained_surface_token,
            ...
            assert_eq!(
                scene
                    .surfaces
                    .iter()
                    .filter_map(|surface| surface.available.then_some(surface.id))
                    .collect::<Vec<_>>(),
                vec![
                    WorkSurfaceId::Files,
                    WorkSurfaceId::Search,
            ...
            prove_outline_surface(&scene, active)?;
        }
        assert_eq!(
            scene_spec("omega_workbench_outline_narrow_foreign_binding")
                .context("registered narrow outline scene")?
                .viewport,
            ViewportFixture::new(910, 720, 2000)
        );
        Ok(())
    ...
    #[test]
    fn outline_fixture_rejects_cross_binding_duplicates_and_invalid_view_state() -> Result<()> {
        let scene = workbench_outline_scene("omega_workbench_outline_combined")?;

        let mut cross_bound = scene.clone();
        cross_bound.outline_snapshots[0].binding.generation += 1;
        assert!(
            cross_bound
                .validate()
                .expect_err("cross-generation outline must fail")
                .to_string()
                .contains("generation/binding")
        );

        let mut duplicated = scene.clone();
        let mut duplicate = duplicated.outline_snapshots[0]
            .items
            .first()
            .context("outline item")?
            .clone();
        duplicate.id = OutlineItemId("another-id".into());
        duplicate.sequence = 99;
        duplicated.outline_snapshots[0].items.push(duplicate);
        duplicated.outline_snapshots[0].virtual_window.total += 1;
        duplicated.outline_snapshots[0].virtual_window.length += 1;
        assert!(
            duplicated
                .validate()
        ...
        let mut invalid_selection = scene.clone();
        invalid_selection.outline_snapshots[0].selected_item_id =
            Some(OutlineItemId("missing".into()));
        assert!(
            invalid_selection
                .validate()
                .expect_err("foreign selection must fail")
                .to_string()
                .contains("selection")
        );

        let mut invalid_window = scene;
        invalid_window.outline_snapshots[0].virtual_window.start = usize::MAX;
        assert!(
            invalid_window
                .validate()
                .expect_err("invalid virtual window must fail")
                .to_string()
                .contains("virtual window")
        );
        Ok(())
    }

    #[test]
    fn outline_projection_property_preserves_identity_and_rejects_invalid_updates() -> Result<()> {
        for revision in 1..=16 {
            let binding = OutlineBindingFixture {
                thread_id: "thread".into(),
                repository_id: "repository".into(),
                worktree_id: "worktree".into(),
                generation: revision,
            };
            let mut projection = OutlineProjection::new(binding.clone(), "retained-outline");
            let observations = outline_base_observations(&binding.worktree_id);
            assert_eq!(
                projection.apply(OutlineUpdate {
                    binding: binding.clone(),
                    revision,
            ...
            let original_ids = projection
                .snapshot()
                .items
                .into_iter()
                .map(|item| ((item.kind, item.source), item.id))
            ...
            assert_eq!(
                projection.apply(OutlineUpdate {
                    binding: binding.clone(),
                    revision: revision + 1,
                    observations: reordered.clone(),
                }),
                OutlineUpdateEffect::Applied
            );
            let accepted = projection.snapshot();
            for item in &accepted.items {
                assert_eq!(
                    original_ids.get(&(item.kind, item.source.clone())),
                    Some(&item.id)
                );
            ...
            assert_eq!(
                ...
                OutlineUpdateEffect::StaleIgnored
            );
            assert_eq!(
                projection.apply(OutlineUpdate {
                    binding: binding.clone(),
            ...
            assert_eq!(
                ...
                OutlineUpdateEffect::ConflictingReplayRejected
            );
            let mut foreign_binding = binding;
            foreign_binding.worktree_id = "foreign-worktree".into();
            assert_eq!(
                projection.apply(OutlineUpdate {
                ...
                OutlineUpdateEffect::ForeignBindingRejected
            );
            let rejected = projection.snapshot();
            assert_eq!(rejected.revision, accepted.revision);
            assert_eq!(rejected.items, accepted.items);
            assert_eq!(rejected.ignored_stale_revision_count, 1);
            assert_eq!(rejected.ignored_replay_count, 1);
            assert_eq!(rejected.rejected_conflicting_revision_count, 1);
            assert_eq!(rejected.rejected_foreign_binding_count, 1);
        }
        Ok(())
    }

    #[test]
    fn outline_filter_selection_anchor_and_virtual_window_reconcile_deterministically() -> Result<()>
    {
        let scene = workbench_outline_scene("omega_workbench_outline_combined")?;
        let binding = scene
            .active_outline_snapshot()
            .context("active outline")?
            .binding
            .clone();
        let mut projection = OutlineProjection::new(binding.clone(), "retained-outline");
        projection.apply(OutlineUpdate {
            binding: binding.clone(),
            revision: 1,
            observations: outline_base_observations(&binding.worktree_id),
        });
        let failed_id = projection
            .snapshot()
            .items
            .into_iter()
            .find(|item| item.status == OutlineItemStatusFixture::Failed)
            .context("failed outline item")?
            .id;
        assert!(projection.select(Some(failed_id.clone())));
        projection.set_virtual_window(1, usize::MAX);
        let combined = projection.snapshot();
        assert_eq!(combined.selected_item_id, Some(failed_id));
        assert_eq!(combined.anchor_item_id, combined.selected_item_id);
        assert_eq!(combined.virtual_window.start, 1);
        assert_eq!(
            combined.virtual_window.length,
            combined.virtual_window.total - 1
        );

        projection.set_view_filter(OutlineViewFixture::Artifacts, OutlineFilterFixture::All);
        let artifacts = projection.snapshot();
        assert_eq!(artifacts.selected_item_id, None);
        assert_eq!(artifacts.anchor_item_id, None);
        assert_eq!(artifacts.virtual_window.total, 2);
        assert!(!projection.select(Some(OutlineItemId("missing".into()))));
        Ok(())
    }

    #[test]
    fn outline_proof_rejects_foreign_item_leaks_and_round_trips_typed_actions() -> Result<()> {
        let scene = workbench_outline_scene("omega_workbench_outline_selected_action")?;
        let actual = scene
            .active_outline_snapshot()
            .context("active outline")?
            .clone();
        let checks = prove_outline_surface(&scene, &actual)?;
        let names = checks
            .iter()
            .map(|check| check.name.as_str())
            .collect::<BTreeSet<_>>();
        for required in [
            ...
            "outline-deduplicated-history",
            "outline-rejected-update-counts",
            "outline-no-foreign-items",
        ] {
            assert!(names.contains(required), "missing outline proof {required}");
        }

        let mut leaked = actual;
        let foreign_item = scene.outline_snapshots[1].items[0].clone();
        leaked.items.push(OutlineItemFixture {
            sequence: 99,
            ..foreign_item
        });
        assert!(prove_outline_surface(&scene, &leaked).is_err());
