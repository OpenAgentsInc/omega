...
mod imp {
    ...
    fn run_enforced(wsl: &Wsl) -> Result<()> {
        ...
        let write_writable = run_in_sandbox(
            &format!("echo omega > {}", shell_quote(&writable_file)),
        ...
        let write_forbidden = run_in_sandbox(
            &format!("echo omega > {}", shell_quote(&forbidden_file)),
            &[],
            default,
        )?;
        ...
        let write_tmp = run_in_sandbox(
            &format!("echo omega > {}", shell_quote(&tmp_path)),
            &[],
            default,
        )?;
        ...
        let write_escape = run_in_sandbox(
            &format!("echo omega > {}", shell_quote(&escape_file)),
            &[],
            fs_write_all,
        )?;
        checks.expect_succeeded(
    ...
    fn check_env_forwarding(checks: &mut Checks) -> Result<()> {
        ...
        env.insert(
            "PATH".to_string(),
            "/omega-sentinel-should-not-win".to_string(),
        );
        let outcome = drive_sandbox(
            "/bin/sh",
            &[
                "-c",
                "[ \"$ZED_TEST_FORWARDED\" = yes ] && [ \"$PATH\" != /omega-sentinel-should-not-win ]",
            ],
            &[],
            SandboxPermissions::default(),
            &env,
