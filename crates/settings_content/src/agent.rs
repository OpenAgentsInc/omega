...
pub struct AgentSettingsContent {
    ...
    /// Whether an authenticated OpenAI API provider may take over a turn when
    /// the ChatGPT Subscription provider's Codex access is unavailable or its
    /// usage is exhausted.
    ///
    /// Default: true
    pub openai_api_fallback: Option<bool>,
    ...
    /// Command to automatically run when Omega creates a Terminal Thread shell in the agent panel.
    ...
    /// external agent's permission system is only used when Omega would allow
...
pub struct SandboxPermissionsContent {
    ...
    /// to without prompting. Paths written by Omega are absolute.
    /// Default: []
