...
pub struct WorkspaceSettingsContent {
    ...
    /// Controls previous session restoration in freshly launched Omega instance.
    ...
    /// Whether to optimize Omega's interface for assistive technology such as
    /// screen readers.
    ///
    /// Default: false
    pub accessible_mode: Option<bool>,
    /// Whether to use the system provided dialogs for Open and Save As.
    /// When set to false, Omega will use the built-in keyboard-first pickers.
    ///
    /// Default: true
    pub use_system_path_prompts: Option<bool>,
    /// Whether to use the system provided prompts.
    /// When set to false, Omega will use the built-in prompts.
    /// Note that this setting has no effect on Linux, where Omega will always
    ...
    /// Controls whether Omega or the window manager or compositor draws window decorations on Linux.
    ///
    /// Default: client
...
/// Configures what draws Omega's window decorations on Linux.
...
pub enum WindowDecorations {
    /// Omega draws its own window decorations/titlebar (client-side decoration).
    #[default]
    Client,
...
pub enum CliDefaultOpenBehavior {
    /// Open directories as a new workspace in the current Omega window's sidebar.
    #[default]
...
pub enum DefaultOpenBehavior {
    /// Open projects in the current Omega window.
    #[default]
    #[strum(serialize = "Add to Existing Window")]
    ExistingWindow,
...
pub enum RestoreOnStartupBehavior {
    ...
    LastWorkspace,
    /// Restore all workspaces that were open when quitting Omega.
    #[default]
    LastSession,
