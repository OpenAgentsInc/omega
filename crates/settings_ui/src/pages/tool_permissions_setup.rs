...
const SETTINGS_DISCLAIMER: &str = "Note: custom tool permissions only apply to the native agent and don’t extend to external agents connected through the Agent Client Protocol (ACP).";
...
mod tests {
    ...
    fn test_all_tools_are_in_tool_info_or_excluded() {
        ...
        const EXCLUDED_TOOLS: &[&str] = &[
            ...
            "market_execute_swap",
            "market_swap_status",
            // Account and market data are read-only. An explicit swap request
            // authorizes the configured LN Markets network.
