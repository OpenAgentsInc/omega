...
const ICON_DIR: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/../omega/resources/windows");
...
pub fn compile(manifest: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (icon_filename, product_name) = match channel {
        "stable" => ("app-icon.ico", "Omega"),
        "preview" => ("app-icon-preview.ico", "Omega RC"),
        "nightly" => ("app-icon-nightly.ico", "Omega Nightly"),
        _ => ("app-icon-dev.ico", "Omega Dev"),
    };
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            VALUE "CompanyName", "OpenAgents, Inc.\0"
            VALUE "LegalCopyright", "Copyright 2026 OpenAgents, Inc.\0"
        END
    END
    BLOCK "VarFileInfo"
    BEGIN
