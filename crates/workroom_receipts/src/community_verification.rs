...
impl CommunityBinding {
    ...
    pub fn bind(mut self, agent_pubkey: &str, operator_pubkey: &str) -> Self {
        self.operator_by_agent.insert(
            agent_pubkey.trim().to_ascii_lowercase(),
            operator_pubkey.trim().to_ascii_lowercase(),
        );
        self
    }
...
pub fn admit_independent_verification(
    ...
    let feedback_type =
        tag_value(event, "cw_feedback_type").or_else(|| tag_value(event, "lbr_feedback_type"));
    ...
    let result_event_id = tagged_event_id(event, "result")
        .unwrap_or_default()
        .to_owned();
