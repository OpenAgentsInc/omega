    ...
    async fn add_view_from_leader(
        ...
        let item = if let Some(existing_item) = existing_item {
            ...
            let Some(task) = task else {
                anyhow::bail!(
                    "failed to construct view from leader (maybe from a different version of Omega?)"
                );
            };
                    ...
                    .update(cx, |_, window, cx| {
                        let detail: SharedString = match err.error_code() {
                            ...
                            ErrorCode::UpgradeRequired => concat!(
                                "Your are running an unsupported version of Omega. ",
...
pub async fn get_any_active_multi_workspace(
    ...
    activate_any_workspace_window(&mut cx).context("could not open Omega")
}
...
pub enum WorkspaceMatching {
    ...
    ///
    /// For example, `omega -a foo/bar` will activate the `bar` workspace if it
