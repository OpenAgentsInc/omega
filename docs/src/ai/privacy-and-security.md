...
including [Omega Agent](./omega-agent.md), [Edit Prediction](./edit-prediction.md),
...
| [Provider API keys](./use-api-access.md)                     | The configured provider                           | The provider handles requests under its own terms. Provider keys saved through Omega are stored in an owner-only local credentials file, not in `settings.json`.                                                              | [Use API Access](./use-api-access.md)                                                             |
